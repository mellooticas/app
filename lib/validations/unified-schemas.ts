...
// =============================================
export const generateLessonMaterialsSchema = z.object({
  lesson_id: z.string().uuid(),
})

export const generateBatchMaterialsSchema = z.object({
  lesson_ids: z.array(z.string().uuid()).min(1).max(50),
})

export const completeAlphaItemSchema = z.object({
  item_id: z.string().uuid(),
})

export const skipAlphaItemSchema = z.object({
  item_id: z.string().uuid(),
})

export const generateNextStepsSchema = z.object({
  user_id: z.string().uuid().optional(), // admin can generate for another user
})
...
export const createLearningPathSchema = z.object({
  ...
  description: z.string().max(5000).optional(),
  cycle: z.enum(['initial', 'fundamental', 'intermediate', 'advanced']).optional(),
  methodology_id: z.string().uuid().optional(),
  instrument_id: z.string().uuid().optional(),
  difficulty_min: z.number().int().min(1).max(5).default(1),
  difficulty_max: z.number().int().min(1).max(5).default(5),
})
...
export const addPathStepSchema = z.object({
  path_id: z.string().uuid(),
  step_order: z.number().int().min(0),
  step_type: z.enum(['lesson', 'challenge', 'portfolio', 'practice', 'ai_content']),
  reference_id: z.string().uuid().optional(),
  title: z.string().min(1).max(200),
  description: z.string().max(2000).optional(),
  is_optional: z.boolean().default(false),
  unlock_after: z.string().uuid().optional(),
  points_reward: z.number().int().min(0).default(10),
})
...
export const createLibraryItemSchema = z.object({
  ...
  description: z.string().max(2000).optional(),
  category: z.enum(['methodology', 'curriculum', 'resource', 'template', 'guide']),
  subcategory: z.string().max(50).optional(),
  file_type: z.enum(['pdf', 'video', 'audio', 'markdown', 'link', 'image']).default('pdf'),
...
export const updateLibraryItemSchema = z.object({
  ...
  description: z.string().max(2000).optional(),
  category: z.enum(['methodology', 'curriculum', 'resource', 'template', 'guide']).optional(),
  subcategory: z.string().max(50).optional(),
  file_type: z.enum(['pdf', 'video', 'audio', 'markdown', 'link', 'image']).optional(),
  file_url: z.string().max(1000).optional(),
  content: z.string().optional(),
  tags: z.array(z.string().max(50)).max(10).optional(),
  is_featured: z.boolean().optional(),
