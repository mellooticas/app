...
import { ChevronRight, Calendar, CheckCircle, ArrowLeft, BookOpen, Lock, RefreshCw, Target, Lightbulb, Globe, Mic, Clapperboard } from 'lucide-react'
import Link from 'next/link'
import { LESSON_LEVELS_YEAR2, STATUS_LABELS, STATUS_COLORS, LEVEL_COLORS } from '@/lib/lessons/constants'
import type { LessonLevelYear2 } from '@/lib/lessons/constants'
...
const prevLevelLinks: Record<string, { href: string; label: string }> = {
  retomada: { href: '/lessons/show-final', label: 'Show Final' },
  aprofundamento: { href: '/lessons/retomada', label: 'Retomada' },
  criacao: { href: '/lessons/aprofundamento', label: 'Aprofundamento' },
  cultura: { href: '/lessons/criacao', label: 'Criacao' },
  performance: { href: '/lessons/cultura', label: 'Cultura' },
  projetos: { href: '/lessons/performance', label: 'Performance' },
}

type StandardLevel = Exclude<LessonLevelYear2, 'formatura'>

export default function Year2LevelPage({ levelKey }: { levelKey: StandardLevel }) {
  const LEVEL = LESSON_LEVELS_YEAR2[levelKey]
  const COLORS = LEVEL_COLORS[LEVEL.color]
  const Icon = levelIcons[levelKey]
  const prevLevel = prevLevelLinks[levelKey]
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Lesson cards or gated state */}
      {isGated ? (
        <div className="text-center py-12 bg-white rounded-2xl border border-gray-100">
          <Lock className="w-10 h-10 text-gray-300 mx-auto mb-3" />
          <p className="text-gray-500 font-medium">Conclua mais aulas para acessar este nivel</p>
          <Link href={prevLevel.href} className="text-sm text-blue-600 hover:underline mt-2 inline-block">
            Ir para {prevLevel.label}
          </Link>
        </div>
      ) : (
          ...
          {lessons.map(lesson => {
            ...
            return (
              <Link key={lesson.id} href={`/lessons/${lesson.id}`} className="block group">
                <div className={`bg-white rounded-xl border shadow-sm p-4 hover:shadow-md transition-all flex items-center gap-4 ${isDone ? 'border-green-200' : `border-gray-100 hover:${COLORS.border}`}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className={`font-bold text-gray-900 group-hover:${COLORS.text} transition-colors`}>{lesson.title}</h3>
