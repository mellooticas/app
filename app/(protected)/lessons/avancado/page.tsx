...
import { TreePine, ChevronRight, Calendar, CheckCircle, ArrowLeft, BookOpen, Lock } from 'lucide-react'
...
const LEVEL = LESSON_LEVELS.avancado
const COLORS = LEVEL_COLORS.red
    ...
    <div className="space-y-6">
      ...
      {isGated ? (
        <div className="text-center py-12 bg-white rounded-2xl border border-gray-100">
          ...
          <Link href="/lessons/intermediario" className="text-sm text-blue-600 hover:underline mt-2 inline-block">
            Ir para Intermediario
          </Link>
        </div>
      ) : (
          ...
          {lessons.map(lesson => {
            ...
            return (
              <Link key={lesson.id} href={`/lessons/${lesson.id}`} className="block group">
                <div className={`bg-white rounded-xl border shadow-sm p-4 hover:shadow-md transition-all flex items-center gap-4 ${isDone ? 'border-red-200' : 'border-gray-100 hover:border-red-200'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-gray-900 group-hover:text-red-700 transition-colors">{lesson.title}</h3>
