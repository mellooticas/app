...
export const useUserAchievements = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!user?.id) throw CommonErrors.AUTH_REQUIRED;
 ...
 */
export const useAchievementsByType = (type: 'badge' | 'trophy' | 'certificate') => {
  const { data: user } = useCurrentUser();
 ...
 */
export const useHasAchievement = (achievementType: string) => {
  const { data: achievements, isLoading } = useUserAchievements();

  const hasAchievement = achievements?.some(
    achievement => achievement.achievement_type === achievementType
  ) || false;
